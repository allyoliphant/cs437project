...
public class Indexing {
	...
	private static void buildIndex() throws IOException {
		...
		PrintWriter writer = new PrintWriter(new FileWriter(new File("src/main/resources/mincollectionIndex.txt")));
		Reader in = new FileReader("src/main/resources/collection/mini-collection.csv");

		// get collection and build initial vocabulary
		System.out.println("get collection/build vocab...");
		Iterable<CSVRecord> records = CSVFormat.EXCEL.withFirstRecordAsHeader().parse(in);
		for (CSVRecord record : records) {
			String id = record.get("id");
			String content = record.get("content");

			System.out.println("on wiki " + id + " of 1,662,756");
		...
		// combine words
		System.out.println("combine vocab...");
		ArrayList<ArrayList<Object>> index = new ArrayList<ArrayList<Object>>();
		for (int i = 0; i < vocab.size(); i++) {
			System.out.println("on token " + (i + 1) + " of " + vocab.size());
		...
		// print to file
		System.out.println("print index to file...");
		for (int i = 0; i < index.size(); i++) {
			writer.print(index.get(i).get(0));
			for (int w = 1; w < index.get(i).size(); w++) {
				writer.print(
						" {" + ((String[]) index.get(i).get(w))[0] + "," + ((String[]) index.get(i).get(w))[1] + "}");
			}
			writer.print("\n");
		}
		writer.close();
	...
	private static void creation() throws IOException {
		...
		PrintWriter writerComplete = new PrintWriter(
				new FileWriter(new File("src/main/resources/creationComplete.txt")));
